BizQQWPA
		.define(
				"util.localStorage",
				"util.cookie,lang.trim",
				function(require) {
					var Cookie = require("util.cookie"), trim = require("lang.trim");
					var COOKIE_PREFIX = "IELS";
					var EXPIRES = 31536e8;
					var doc = document, commonPattern = new RegExp("(?:^|[ ;])"
							+ COOKIE_PREFIX + "[^=]+=([^;$])"), keyPattern = function(
							key) {
						...
						if (callback) {
							...
							for (; i < length; i++) {
								(match = commonPattern.exec(attributes[i]))
										&& items.push(match[1])
							}
						}
						return items
					...
					return window.localStorage
							|| {
								...
								setItem : function(key, value) {
									Cookie.set(keyPattern(key), value, null,
											"/", EXPIRES)
								},
				...
				function(require) {
					...
					var SETTINGS = {
						...
						template : [
								...
								'<p class="WPA3-SELECT-PANEL-GUIDE">璇烽€夋嫨鍙戣捣鑱婂ぉ鐨勬柟寮忥細</p>',
								...
								'<span class="WPA3-SELECT-PANEL-LABEL">QQ甯愬彿鑱婂ぉ</span>',
								"</a>",
								...
								'<span class="WPA3-SELECT-PANEL-LABEL">鍖垮悕鑱婂ぉ</span>',
								"</a>",
								"</div>",
								"</div>",
								'<div class="WPA3-SELECT-PANEL-BOTTOM">',
								'<a target="_blank" href="http://im.qq.com" class="WPA3-SELECT-PANEL-INSTALL">瀹夎QQ</a>',
								"</div>", "</div>" ].join(""),
						cssText : [
