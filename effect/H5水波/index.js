...
function init() {
	...
	water.onmousedown = function(e) {
		...
		if (window.innerHeight / 2 - mouseY < 50
				&& window.innerHeight / 2 - mouseY > -50)
		// diffPt[150] = autoDiff;
		{
			autoDiff = 1000;
			if (mouseX < water.width - 2) {
				xx = 1 + Math.floor((verNum - 2) * mouseX / water.width);
...
function draw() {
	...
	ctx.lineTo(water.width, window.innerHeight);
	...
	ctx.lineTo(water.width, water.innerHeight);
	ctx.lineTo(0, window.innerHeight);
	ctx.fill();
	...
	ctx.fillText("点击该液体的表面", 70, water.height / 2 - 20);
	ctx.fillStyle = "#fff";
	ctx.fillText("用鼠标滚轮来改变粘度", 70, water.height / 2 + 15);
	ctx.fillText("滚轮改变粘稠度 / Viscosity: " + ((dd - 15) * 20 / 7).toFixed(2)
			+ "%", 70, water.height - 20);
}
